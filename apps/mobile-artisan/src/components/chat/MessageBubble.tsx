...
function AudioPlayer({ uri, isMine }: { uri: string; isMine: boolean }) {
  ...
  return (
    <Pressable onPress={() => void toggle()} style={styles.audio}>
      <Text style={[styles.audioIcon, !isMine && styles.audioIconOther]}>
        {playing ? "⏸" : "▶"}
      </Text>
      <Text style={[styles.audioLabel, !isMine && styles.audioLabelOther]}>
        Message vocal
      </Text>
    </Pressable>
  );
    ...
    <View style={[styles.row, isMine && styles.rowMine]}>
      <View style={[styles.bubble, isMine ? styles.bubbleMine : styles.bubbleOther]}>
        ...
        {message.type === "AUDIO" && message.fileUrl ? (
          <AudioPlayer uri={message.fileUrl} isMine={isMine} />
        ) : null}
        {message.content ? (
...
const styles = StyleSheet.create({
  ...
  audioIconOther: { color: "#15803d" },
  audioLabel: { color: "#fff", fontWeight: "600" },
  audioLabelOther: { color: "#14532d" },
  meta: {
    flexDirection: "row",
    alignItems: "center",
